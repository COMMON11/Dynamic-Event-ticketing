    ...
    const fetchEventDetails = async () => {
      try {
        const response = await axios.get(
          `/api/getEventById?event_id=${id}&user_id=${userId}`
        );
  ...
  return (
    <>
      ...
      <div className="font-display flex flex-col items-center mt-10">
        <div className="w-[70%] bg-gray-100 border-4 border-gray-400 rounded-2xl min-h-screen">
          <img
            ...
            className="w-full h-72 rounded-t-xl border-4 border-gray-400"
          />
          <img
            src={`data:${event.logoType};base64,${event.logo}`}
            alt={event.event_name}
            className="inset-0 w-40 h-40 rounded-full border-8 border-gray-400 -translate-y-20 translate-x-12"
          />
          <h2 className="text-4xl ml-12 -translate-y-16 h-14 rounded-2xl font-bold">
            {event.event_name}
          </h2>
          <div className="ml-12 -translate-y-12 mr-12 w-auto">
            <label className="text-3xl ">About this event:</label>
            <br />
            <p className="rounded-xl bg-gray-300 p-2">{event.description}</p>
          </div>
          <div className="-translate-y-6 flex w-full justify-between">
            <p className="ml-12 border-black border-dashed border-2 rounded-xl p-2 w-fit text-2xl">
              Due: {event.due_date}
            </p>
            <p className="text-2xl mr-12 p-2">
      ...
      <footer className="font-display fixed bottom-0 left-0 right-0 w-full bg-white border-t-8 border-gray-400 flex-col min-h-16 p-2">
        {message && (
          <p
            className={`${messageStyle} font-bold text-lg text-center mt-2 mb-2`}
          >
            {message}
          </p>
        )}
        <div className="flex items-center justify-center">
          <p className="font-bold mr-4 border-dashed borde-2">
            Total Price: ₹{params.cost}
          </p>
          ...
          <div className="flex justify-center">
            <div className="relative">
              <input
                ...
                disabled={disableBooking || submitLoading}
                className="bg-redishpink-100 w-32 h-12 relative z-10 transition duration-75 ease-in-out border-2 border-red-500 text-white active:translate-x-2 active:translate-y-2 hover:border-4 hover:border-red-600 disabled:bg-gray-500 disabled:border-gray-800"
              />
        ...
        <div className="w-full flex justify-center">
          {enableEdit && (
            <Link to={`/event/edit/${event.event_id}`}>
              <input
                type="button"
                value="Edit event"
                className="bg-gray-500 p-2 rounded-lg text-white border-2 border-gray-400 transistion duration-200 ease-in-out hover:border-4 hover:border-gray-700"
              />
            </Link>
          )}
        </div>
